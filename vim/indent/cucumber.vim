...
function! GetCucumberIndent()
	...
	" kidonchu addition
	" reset indent to correct level if previous line is empty
	let prevLine = getline(v:lnum-1)
	let prevPrevLine = getline(v:lnum-2)
	if (csyn ==# 'cucumberExamples' || cline =~# '^\s*Examples:')
		return indent(prevnonblank(v:lnum-1))
	elseif prevLine =~# '^\s*$' && !(csyn ==# 'cucumberFeature' || cline =~# '^\s*Feature:')
		return sw
